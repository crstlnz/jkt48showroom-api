...
import { getTheaterList } from '../jkt48/theater'
...
export async function getMemberDetails(slug: string): Promise<IMemberProfileAPI> {
  ...
  let recentTheater: IApiTheaterInfo[] = []
  let upcomingTheater: IApiTheaterInfo[] = []
  ...
  if (data.group === 'jkt48' && data.jkt48id) {
    ...
    //     id: i.id,
    //     name: i.title,
    //     date: (i.start_time ?? i.date ?? new Date(0)).toISOString(),
    //     url: getNewTheaterUrl(i.code),
